...
outpath  = opj(basepath, 'derivatives/glms/model_offer1')
...
def run_first(model, img, event, conf, outpath, confounds_use):
    ...
    for run, (ev, cf) in enumerate(zip(event, conf)):
        ...
        for tt, pfirst ,plev, mlev in zip(ev_o['trial_type'], ev_o['painfirst'],
                                          ev_o['pain_rank'], ev_o['money_rank']):
                if tt == 'offer1_onset':
                    to_keep.append(1)
                    if pfirst:
                        new_types.append(str('o1_pain_level_' + str(int(plev))))
                        level.append(plev)
                    else:
                        new_types.append(str('o1_money_level_' + str(int(mlev))))
                        level.append(mlev)
                else:
                    to_keep.append(0)
                    new_types.append(tt)
