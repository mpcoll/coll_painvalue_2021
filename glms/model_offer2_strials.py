...
# basepath = '/data'
# Uses as lot of memory so might want to keep this relatively low
...
outpath  = opj(basepath, 'derivatives/glms/model_offer2')
...
def run_first(model, img, event, conf, outpath, confounds_use):
    ...
    for run, (ev, cf) in enumerate(zip(event, conf)):
        ...
        new_types, to_keep = [], []
        ev_o['trial_type_old'] = ev_o['trial_type'][:]


        # Rename trials of interest to model separately
        for tt, tr ,runtrial, acc, plev, mlev in zip(ev_o['trial_type'], ev_o['trials'], ev_o['run'],
                                                ev_o['accept'], ev_o['pain_rank'], ev_o['money_rank']):
            if tt == 'offer2_onset':
                to_keep.append(1)
                new_types.append(str('run_') + str(int(runtrial)) + '_trial'
                                    + str(tr).zfill(3) + '_' + tt + '_p' + str(int(plev))
                                    + '_m' + str(int(mlev)) + '_a' + str(int(acc)))
            else:
                to_keep.append(0)
                new_types.append(tt)
    ...
    report = model_fit.generate_report('offer1_onset')
    report.save_as_html(opj(outpath, 'sub-' + s + '_glm_report.html'))
    img_cont = None # Clear memory

    # Calculate vif for single trials
    vifs = []
    for ridx, d in enumerate(model_fit.design_matrices_):
        polys = [c for c in list(d.columns) if 'drift'in c] + [c for c in list(d.columns) if 'const_'in c] + ['constant']
        dm = Design_Matrix(d, sampling_freq=1/model_fit.t_r,
                            polys=polys)


        vframe = pd.DataFrame(dict(vif=dm.vif(), run=ridx+1,
                                    regressor=list(dm.columns)[:-len(dm.polys)]))
        vframe = vframe[vframe['regressor'].str.match('run_')].reset_index()
        vframe['trial'] = np.arange(1, len(vframe)+1)
        vifs.append(vframe)


    # Keep only image events
    events_all = pd.concat(events_out)
    vifs = pd.concat(vifs)
    events_save = events_all[events_all['to_keep'] == 1]
    assert np.array_equal(np.asarray(events_save['trial_type']),
                          np.asarray(vifs['regressor']))
    events_save['vifs'] = np.asarray(vifs['vif'])
    events_save.loc[:, 'subject_id'] = 'sub-' + s
    assert len(events_save) == 100  # Sanity check
